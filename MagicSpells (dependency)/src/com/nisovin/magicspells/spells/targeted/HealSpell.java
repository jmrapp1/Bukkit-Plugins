...
public class HealSpell extends TargetedSpell implements TargetedEntitySpell {
	...
	public HealSpell(MagicConfig config, String spellName) {
		...
		checker = new ValidTargetChecker() {
			...
			public boolean isValidTarget(LivingEntity entity) {
				...
				return d.getHealth() < d.getMaxHealth();
			}
		};
	}
	...
	public PostCastAction castSpell(Player player, SpellCastState state, float power, String[] args) {
		if (state == SpellCastState.NORMAL) {
			...
			if (cancelIfFull && d.getHealth() == d.getMaxHealth()) {
	...
	private boolean heal(Player player, LivingEntity target, float power) {
		...
		health += amt;
		Damageable da = (Damageable) target;
		if (health > da.getMaxHealth()) health = da.getMaxHealth();
	...
	public boolean castAtEntity(Player caster, LivingEntity target, float power) {
		Damageable d = (Damageable) target;
		if (validTargetList.canTarget(caster, target) && d.getHealth() < d.getMaxHealth()) {
	...
	public boolean castAtEntity(LivingEntity target, float power) {
		Damageable d = (Damageable) target;
		if (validTargetList.canTarget(target) && d.getHealth() < d.getMaxHealth()) {
