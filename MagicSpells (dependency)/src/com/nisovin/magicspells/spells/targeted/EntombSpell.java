...
public class EntombSpell extends TargetedSpell implements TargetedEntitySpell {
	...
	public PostCastAction castSpell(Player player, SpellCastState state, float power, String[] args) {
		if (state == SpellCastState.NORMAL) {
			...
			if (targetInfo != null) {
				...
				createTomb(player, target, power);
	...
	private void createTomb(Player player, LivingEntity target, float power) {		
		ArrayList<Block> tombBlocks = new ArrayList<Block>();
		Block feet = target.getLocation().getBlock();
		
		Minigame m = player != null ? MinigameManager.getInstance().getMinigamesIn(player) : null;
		...
		if (temp.getType() == Material.AIR) {
			...
			tombBlocks.add(temp);
			if (m != null)
				m.getRegion().addBlock(temp.getLocation(), Material.AIR, temp.getData());
		}
	...
	public boolean castAtEntity(Player caster, LivingEntity target, float power) {
		if (!validTargetList.canTarget(caster, target)) {
			...
			createTomb(caster, target, power);
