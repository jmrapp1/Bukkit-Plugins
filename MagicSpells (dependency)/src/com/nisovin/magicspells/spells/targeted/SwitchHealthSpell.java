...
public class SwitchHealthSpell extends TargetedSpell implements TargetedEntitySpell {
	...
	boolean switchHealth(Player caster, LivingEntity target) {
		...
		Damageable d1 = (Damageable) caster;
		Damageable d2 = (Damageable) target;
		double casterPct = d1.getHealth() / d1.getMaxHealth();
		double targetPct = d2.getHealth() / d2.getMaxHealth();
		...
		caster.setHealth(targetPct * d1.getMaxHealth());
		target.setHealth(casterPct * d2.getMaxHealth());
