...
public class CastCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String [] args) {
		try {
			...
			if (command.getName().equalsIgnoreCase("grantspell")) {
				if (sender instanceof Player) {
					Player player = (Player) sender;
					if (player.hasPermission("magicspells.grantspell")) {
						if (args.length == 2) {
							Tuple<Player, Integer> result = PlayerManager.getInstance().findPlayerAndCount(args[0]);
							if (result != null) {
								if (result.b() == 1) {
									Player target = result.a();
									Spellbook spellbook = MagicSpells.getSpellbook(target);
									Spell spell = MagicSpells.getSpellByInternalName(args[1]);
									if (spell == null)
									...
									if (spell != null) {
										if (!spellbook.hasSpell(spell)) {
											...
											spellbook.save();
											player.sendMessage(ChatColor.GREEN + "You have granted the player \"" + spell.getName() + "\".");
											target.sendMessage(ChatColor.AQUA + "You have been granted the \"" + spell.getName() + "\" spell!");
										} else {
											player.sendMessage(ChatColor.RED + "The player already owns that spell.");
										}
									} else {
										player.sendMessage(ChatColor.RED + "No spell was found with that name.");
									}
								} else {
									player.sendMessage(ChatColor.RED + "More than one player was found with that name.");
								}
							} else {
								player.sendMessage(ChatColor.RED + "No player was found.");
							}
						} else {
							player.sendMessage(ChatColor.RED + "Usage: /grantspell <player> <spellName>");
						}
					} else {
				...
				if (sender instanceof Player) {
					Player player = (Player) sender;
					if (player.hasPermission("magicspells.removespell")) {
						if (args.length == 2) {
							Tuple<Player, Integer> result = PlayerManager.getInstance().findPlayerAndCount(args[0]);
							if (result != null) {
								if (result.b() == 1) {
									...
									if (spell != null) {
										if (spellbook.hasSpell(spell)) {
											...
											spellbook.save();
											player.sendMessage(ChatColor.GREEN + "You have removed the player's spell.");
											target.sendMessage(ChatColor.RED + "Your \"" + spell.getName() + "\" spell has been removed by " + player.getDisplayName() + ".");
										} else {
											player.sendMessage(ChatColor.RED + "The player does not own that spell.");
										}
									} else {
										player.sendMessage(ChatColor.RED + "No spell was found with that name.");
									}
							...
							player.sendMessage(ChatColor.RED + "Usage: /removespell <player> <spellName>");
						}
					} else {
						player.sendMessage(ChatColor.DARK_RED + "You don't have permission to use this command.");
					}
			...
			} else if (command.getName().equalsIgnoreCase("spells")) {
				if (sender instanceof Player) {
					Player player = (Player) sender;
					if (player.hasPermission("magicspells.spelllist")) {
						String spells = "";
						for (Spell spell : MagicSpells.getSpellbook(player).getSpells()) {
							spells += spell.getName() + ", ";
						}
						spells = spells.substring(0, spells.length() - 2);
						player.sendMessage(ChatColor.DARK_AQUA + "Spells Known: " + spells);
					}
				}
			} else if (command.getName().equalsIgnoreCase("magicspellcast")) {
				if (args == null || args.length == 0) {
					...
					Player player = (Player)sender;
					if (player.hasPermission("chatcast")) {
						...
						MagicSpells.sendMessage(player, ChatColor.RED + "You can't cast spells through commands.");
						return true;
					}
				} /*else { // not a player
