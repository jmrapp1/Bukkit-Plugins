...
public class Spellbook {
	...
	private void loadFromPlayerInfo() {
		PlayerInformation info = PlayerManager.getInstance().getInfo(player);
		for (String spellName : info.getOwnedSpells()) {
			Spell spell = MagicSpells.getSpellByInternalName(spellName);
			if (spell != null) {
				addSpell(spell, spell.getCastItems());
			}
		}
	}
	...
	public boolean canLearn(Spell spell) {
		...
		if (spell.prerequisites != null) {
			for (String spellName : spell.prerequisites) {
				...
				PlayerInformation info = PlayerManager.getInstance().getInfo(player);
				if (sp == null || !hasSpell(sp) || !info.ownsSpell(sp)) {
					return false;
				}
			}
		}
	...
	public boolean canCast(Spell spell) {
		...
		if (player.hasMetadata("magicClass")) {
			...
			return plugin.ignoreCastPerms || /*player.hasPermission("magicspells.cast." + spell.getPermissionName())*/ (info.ownsSpell(spell) && mc.hasSpell(spell));
		} else {
			return false;
		}
	}
			...
			if (spells.size() > 1 || i.equals(-1) || plugin.allowCycleToNoSpell || plugin.alwaysShowMessageOnCycle) {
				...
				while (count++ < spells.size()) {
					...
					if (!plugin.onlyCycleToCastableSpells || canCast(spells.get(i))) {
						if (!MagicSpells.itemIsReplaced(player, spells.get(i))) {
	...
	public void setActiveSpell(ItemStack item, Spell spell) {
		CastItem castItem = new CastItem(item);
		ArrayList<Spell> spells = itemSpells.get(castItem);
		if (spells != null) {
			int index = 0;
			for (int i = 0; i < spells.size(); i++) {
				if (spells.get(i).getName() == spell.getName()) {
					index = i;
					break;
				}
			}
			activeSpells.put(castItem, index);
		}
	}
	...
	public boolean hasSpell(Spell spell, boolean checkGranted) {
		...
		PlayerInformation info = PlayerManager.getInstance().getInfo(player);
		boolean has = allSpells.contains(spell) || info.ownsSpell(spell);
		if (has) {
			return true;
	...
	public void addSpell(Spell spell, CastItem[] castItems) {
		...
		if (spell.replacedBy != null) {
			for (String spellName : spell.replacedBy) {
				Spell sp = MagicSpells.getSpellByInternalName(spellName);
